...
describe('#Specific Letters', () => {
  it('Debe evaluar sí un array de strings contiene strings de la misma dimensión', () => {
    const hasSameDimentions = specificLetters.hasSameDimentions(validDnaArray);
    assert.equal(hasSameDimentions, true);
  });

  it('Debe evaluar sí un array de strings Es válido (contiene sólo A,C,T,G)', () => {
    const isValidDnaArray = specificLetters.isValidDnaArray(validDnaArray);
    assert.equal(isValidDnaArray, true);
  });

  it('Debe regresar un booleano falso,porque el array no es válido (contiene sólo A,C,T,G)', () => {
